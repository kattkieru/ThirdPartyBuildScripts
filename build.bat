...
set FABRIC_CFLAGS=/D_SCL_SECURE_NO_WARNINGS=1 /D_ITERATOR_DEBUG_LEVEL=0 /D_SECURE_SCL=0
...
set BULLET_NAME=bullet-2.78
set LASZIP_NAME=laszip-2.0.2
set LAS_NAME=libLAS-1.7.0b2
set JPEG_TURBO_NAME=libjpeg-turbo-1.2.1
...
set LIB_ARCH_DEBUG_DIR=%TOP%\PreBuilt\Windows\%ARCHTARGETDIR%\Debug\lib
set LIB_ARCH_RELEASE_DIR=%TOP%\PreBuilt\Windows\%ARCHTARGETDIR%\Release\lib
set INCLUDE_ARCH_DEBUG_DIR=%TOP%\PreBuilt\Windows\%ARCHTARGETDIR%\Debug\include
set INCLUDE_ARCH_RELEASE_DIR=%TOP%\PreBuilt\Windows\%ARCHTARGETDIR%\Release\include
...
rem ============ RLM =============
if "%ARCH%" == "x64" (
  echo Only build RLM for x86 ARCH, skipping
)
else (
  echo rlm - Building
  cd Build
  mkdir rlm
  mkdir rlm\arch
  cp ..\PreBuilt\Windows\x86\Release\lib\rlm\* rlm\arch\
  mkdir rlm\src
  cp ..\include\rlm\rlm\* rlm\src\
  cp ..\rlm\* rlm\src\
  cd rlm\src
  sed -i "s/#include \"rlm\//#include \"/" rlm_isv_config.c
  cd ..\arch
  sed -i "s/ISV.=.*/ISV = fabricinc/" makefile
  sed -i "s/MACHINE.=.*/MACHINE = x86/" makefile
  sed -i "s/^!/#!/" makefile
  make rlmsign.exe rlm.exe fabricinc.set rlmutil.exe
  zip -r rlm_fabricinc_win32.zip rlm.exe rlmutil.exe fabricinc.set
  mv rlm_fabricinc_win32.zip ..\..
)

rem ============ JPEG_TURBO =============
if EXIST %CHECKPOINT_DIR%\jpeg_turbo_unpack goto jpeg_turbo_unpack_done
echo jpeg_turbo - Unpacking
cd %BUILD_DIR%
type "%TOP%\SourcePackages\%JPEG_TURBO_NAME%.tar.gz" | gzip -d -c | tar -x -f- 2> NUL:
touch %CHECKPOINT_DIR%\jpeg_turbo_unpack
:jpeg_turbo_unpack_done

if EXIST %CHECKPOINT_DIR%\jpeg_turbo_compile_debug goto jpeg_turbo_compile_debug_done
echo jpeg_turbo - Compiling debug
cd %BUILD_DIR%\%JPEG_TURBO_NAME%
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Debug' -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%"
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreadedDebug<\/RuntimeLibrary>/" jpeg-static.vcxproj >jpeg-static.vcxproj.new
mv jpeg-static.vcxproj.new jpeg-static.vcxproj
devenv ALL_BUILD.vcxproj /build "Debug|%VSARCH%" /project jpeg-static
rem touch %CHECKPOINT_DIR%\jpeg_turbo_compile_debug
:jpeg_turbo_compile_debug_done

if EXIST %CHECKPOINT_DIR%\jpeg_turbo_compile_release goto jpeg_turbo_compile_release_done
echo jpeg_turbo - Compiling release
cd %BUILD_DIR%\%JPEG_TURBO_NAME%
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Release' -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%"
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreaded<\/RuntimeLibrary>/" jpeg-static.vcxproj >jpeg-static.vcxproj.new
mv jpeg-static.vcxproj.new jpeg-static.vcxproj
devenv ALL_BUILD.vcxproj /build "Release|%VSARCH%" /project jpeg-static
touch %CHECKPOINT_DIR%\jpeg_turbo_compile_release
:jpeg_turbo_compile_release_done

if EXIST %CHECKPOINT_DIR%\jpeg_turbo_install goto jpeg_turbo_install_done
echo jpeg_turbo - Installing
cd %BUILD_DIR%\%JPEG_TURBO_NAME%
if NOT EXIST %LIB_ARCH_DEBUG_DIR%\jpeg-turbo mkdir %LIB_ARCH_DEBUG_DIR%\jpeg-turbo
echo F | xcopy Debug\jpeg-static.lib %LIB_ARCH_DEBUG_DIR%\jpeg-turbo\jpeg.lib /Y > NUL:
if NOT EXIST %INCLUDE_ARCH_DEBUG_DIR%\jpeg-turbo mkdir %INCLUDE_ARCH_DEBUG_DIR%\jpeg-turbo
if NOT EXIST %INCLUDE_ARCH_DEBUG_DIR%\jpeg-turbo\jpeg mkdir %INCLUDE_ARCH_DEBUG_DIR%\jpeg-turbo\jpeg
cp *.h %INCLUDE_ARCH_DEBUG_DIR%\jpeg-turbo\jpeg
if NOT EXIST %LIB_ARCH_RELEASE_DIR%\jpeg-turbo mkdir %LIB_ARCH_RELEASE_DIR%\jpeg-turbo
echo F | xcopy Release\jpeg-static.lib %LIB_ARCH_RELEASE_DIR%\jpeg-turbo\jpeg.lib /Y > NUL:
if NOT EXIST %INCLUDE_ARCH_RELEASE_DIR%\jpeg-turbo mkdir %INCLUDE_ARCH_RELEASE_DIR%\jpeg-turbo
if NOT EXIST %INCLUDE_ARCH_RELEASE_DIR%\jpeg-turbo\jpeg mkdir %INCLUDE_ARCH_RELEASE_DIR%\jpeg-turbo\jpeg
cp *.h %INCLUDE_ARCH_RELEASE_DIR%\jpeg-turbo\jpeg
touch %CHECKPOINT_DIR%\jpeg_turbo_install
:jpeg_turbo_install_done


rem ============ LASZIP =============
if EXIST %CHECKPOINT_DIR%\laszip_unpack goto laszip_unpack_done
echo laszip - Unpacking
cd %BUILD_DIR%
unzip "%TOP%\SourcePackages\%LASZIP_NAME%.zip"
touch %CHECKPOINT_DIR%\laszip_unpack
:laszip_unpack_done

if EXIST %CHECKPOINT_DIR%\laszip_compile_debug goto laszip_compile_debug_done
echo laszip - Compiling debug
cd %BUILD_DIR%\%LASZIP_NAME%
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Debug' -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS%" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%" -DBUILD_STATIC=1
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreadedDebug<\/RuntimeLibrary>/" src/laszip.vcxproj >src/laszip.vcxproj.new
mv src/laszip.vcxproj.new src/laszip.vcxproj
devenv ALL_BUILD.vcxproj /build "Debug|%VSARCH%" /project laszip
rem touch %CHECKPOINT_DIR%\laszip_compile_debug
:laszip_compile_debug_done

if EXIST %CHECKPOINT_DIR%\laszip_compile_release goto laszip_compile_release_done
echo laszip - Compiling release
cd %BUILD_DIR%\%LASZIP_NAME%
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Release' -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS%" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%" -DBUILD_STATIC=1
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreaded<\/RuntimeLibrary>/" src/laszip.vcxproj >src/laszip.vcxproj.new
mv src/laszip.vcxproj.new src/laszip.vcxproj
devenv ALL_BUILD.vcxproj /build "Release|%VSARCH%" /project laszip
touch %CHECKPOINT_DIR%\laszip_compile_release
:laszip_compile_release_done

if EXIST %CHECKPOINT_DIR%\laszip_install goto laszip_install_done
echo laszip - Installing
cd %BUILD_DIR%\%LASZIP_NAME%
if NOT EXIST %LIB_ARCH_DEBUG_DIR%\laszip mkdir %LIB_ARCH_DEBUG_DIR%\laszip
echo F | xcopy bin\Debug\Debug\laszip.lib %LIB_ARCH_DEBUG_DIR%\laszip\ /Y > NUL:
if NOT EXIST %LIB_ARCH_RELEASE_DIR%\laszip mkdir %LIB_ARCH_RELEASE_DIR%\laszip
echo F | xcopy bin\Release\Release\laszip.lib %LIB_ARCH_RELEASE_DIR%\laszip\ /Y > NUL:
touch %CHECKPOINT_DIR%\laszip_install
:laszip_install_done

rem ============ LAS =============
if EXIST %CHECKPOINT_DIR%\las_unpack goto las_unpack_done
echo las - Unpacking
cd %BUILD_DIR%
unzip "%TOP%\SourcePackages\%LAS_NAME%.zip"
type "%TOP%\Patches\Windows\%LAS_NAME%-patch.tar.bz2" | bzip2 -d -c | tar -x -f- 2> NUL:
touch %CHECKPOINT_DIR%\las_unpack
:las_unpack_done

if EXIST %CHECKPOINT_DIR%\las_compile_debug goto las_compile_debug_done
echo las - Compiling debug
cd %BUILD_DIR%\%LAS_NAME%
REM libLAS doesn't like the way our Boost libs are named
mkdir ..\boost_libs_debug_%ARCH%
cp %LIB_ARCH_DEBUG_DIR%\boost\libboost_program_options*.lib ..\boost_libs_debug_%ARCH%\libboost_program_options.lib
cp %LIB_ARCH_DEBUG_DIR%\boost\libboost_thread*.lib ..\boost_libs_debug_%ARCH%\libboost_thread.lib
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Debug' -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS% /EHsc" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS% /EHsc" -DWITH_LASZIP=1 -DLASZIP_INCLUDE_DIR=%TOP%\include -DLASZIP_LIBRARY=%LIB_ARCH_DEBUG_DIR%\laszip\laszip.lib -DBoost_USE_STATIC_LIBS=1 -DBoost_INCLUDE_DIR=%TOP%\include\boost -DBOOST_LIBRARYDIR=../boost_libs_debug_%ARCH%
REM cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Debug' -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS% /EHsc" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS% /EHsc" -DWITH_LASZIP=1 -DLASZIP_INCLUDE_DIR=%TOP%\include -DLASZIP_LIBRARY=%LIB_ARCH_DEBUG_DIR%\laszip\laszip.lib -DBoost_USE_STATIC_LIBS=1 -DBoost_INCLUDE_DIR=%TOP%\include\boost -DBOOST_LIBRARYDIR=%LIB_ARCH_DEBUG_DIR%\boost\
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreadedDebug<\/RuntimeLibrary>/" src/liblas.vcxproj >src/liblas.vcxproj.new
mv src/liblas.vcxproj.new src/liblas.vcxproj
devenv ALL_BUILD.vcxproj /build "Debug|%VSARCH%" /project liblas
touch %CHECKPOINT_DIR%\las_compile_debug
:las_compile_debug_done

if EXIST %CHECKPOINT_DIR%\las_compile_release goto las_compile_release_done
echo las - Compiling release
cd %BUILD_DIR%\%LAS_NAME%
REM libLAS doesn't like the way our Boost libs are named
mkdir ..\boost_libs_release_%ARCH%
cp %LIB_ARCH_RELEASE_DIR%\boost\libboost_program_options*.lib ..\boost_libs_release_%ARCH%\libboost_program_options.lib
cp %LIB_ARCH_RELEASE_DIR%\boost\libboost_thread*.lib ..\boost_libs_release_%ARCH%\libboost_thread.lib
cmake . -G %CMAKEGENERATOR% -DCMAKE_BUILD_TYPE='Release' -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS% /EHsc" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS% /EHsc" -DWITH_LASZIP=1 -DLASZIP_INCLUDE_DIR=%TOP%\include -DLASZIP_LIBRARY=%LIB_ARCH_RELEASE_DIR%\laszip\laszip.lib -DBoost_USE_STATIC_LIBS=1 -DBoost_INCLUDE_DIR=%TOP%\include\boost -DBOOST_LIBRARYDIR=../boost_libs_release_%ARCH%
sed -e "s/<RuntimeLibrary>.*<\/RuntimeLibrary>/<RuntimeLibrary>MultiThreaded<\/RuntimeLibrary>/" src/liblas.vcxproj >src/liblas.vcxproj.new
mv src/liblas.vcxproj.new src/liblas.vcxproj
devenv ALL_BUILD.vcxproj /build "Release|%VSARCH%" /project liblas
touch %CHECKPOINT_DIR%\las_compile_release
:las_compile_release_done

if EXIST %CHECKPOINT_DIR%\las_install goto las_install_done
echo las - Installing
cd %BUILD_DIR%\%LAS_NAME%
if NOT EXIST %LIB_ARCH_DEBUG_DIR%\las mkdir %LIB_ARCH_DEBUG_DIR%\las
echo F | xcopy bin\Debug\Debug\liblas.lib %LIB_ARCH_DEBUG_DIR%\las\las.lib /Y > NUL:
if NOT EXIST %LIB_ARCH_RELEASE_DIR%\las mkdir %LIB_ARCH_RELEASE_DIR%\las
echo F | xcopy bin\Release\Release\liblas.lib %LIB_ARCH_RELEASE_DIR%\las\las.lib /Y > NUL:
touch %CHECKPOINT_DIR%\las_install
:las_install_done
...
if "%ARCH%" == "x64" (
  set GLUT_LIB=glut64.lib
)

if EXIST %CHECKPOINT_DIR%\bullet_unpack goto bullet_unpack_done
echo bullet - Unpacking
cd %BUILD_DIR%
unzip "%TOP%\SourcePackages\%BULLET_NAME%.zip"
touch %CHECKPOINT_DIR%\bullet_unpack
:bullet_unpack_done

if EXIST %CHECKPOINT_DIR%\bullet_compile_debug goto bullet_compile_debug_done
echo bullet - Compiling debug
cd %BUILD_DIR%\%BULLET_NAME%
cmake . -G %CMAKEGENERATOR% -DBUILD_EXTRAS='OFF' -DBUILD_DEMOS='OFF' -DCMAKE_BUILD_TYPE='Debug' -DGLUT_glut_LIBRARY=%LIB_ARCH_DEBUG_DIR%\glut\%GLUT_LIB% -DGLUT_INCLUDE_DIR=%INCLUDE_ARCH_DEBUG_DIR%\glut\ -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS%" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%"
devenv ALL_BUILD.vcxproj /build "Debug|%VSARCH%"
touch %CHECKPOINT_DIR%\bullet_compile_debug
:bullet_compile_debug_done

if EXIST %CHECKPOINT_DIR%\bullet_compile_release goto bullet_compile_release_done
echo bullet - Compiling release
cd %BUILD_DIR%\%BULLET_NAME%
cmake . -G %CMAKEGENERATOR% -DBUILD_EXTRAS='OFF' -DBUILD_DEMOS='OFF' -DCMAKE_BUILD_TYPE='Release' -DGLUT_glut_LIBRARY=%LIB_ARCH_RELEASE_DIR%\glut\%GLUT_LIB% -DGLUT_INCLUDE_DIR=%INCLUDE_ARCH_RELEASE_DIR%\glut\ -DCMAKE_CXX_FLAGS="%FABRIC_CFLAGS%" -DCMAKE_C_FLAGS="%FABRIC_CFLAGS%"
devenv ALL_BUILD.vcxproj /build "Release|%VSARCH%"
touch %CHECKPOINT_DIR%\bullet_compile_release
:bullet_compile_release_done

if EXIST %CHECKPOINT_DIR%\bullet_install goto bullet_install_done
echo bullet - Installing
cd %BUILD_DIR%\%BULLET_NAME%
if NOT EXIST %LIB_ARCH_DEBUG_DIR%\bullet mkdir %LIB_ARCH_DEBUG_DIR%\bullet
echo F | xcopy lib\Debug\BulletCollision.lib %LIB_ARCH_DEBUG_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Debug\BulletSoftBody.lib %LIB_ARCH_DEBUG_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Debug\BulletDynamics.lib %LIB_ARCH_DEBUG_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Debug\LinearMath.lib %LIB_ARCH_DEBUG_DIR%\bullet\ /Y > NUL:
if NOT EXIST %LIB_ARCH_RELEASE_DIR%\bullet mkdir %LIB_ARCH_RELEASE_DIR%\bullet
echo F | xcopy lib\Release\BulletCollision.lib %LIB_ARCH_RELEASE_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Release\BulletSoftBody.lib %LIB_ARCH_RELEASE_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Release\BulletDynamics.lib %LIB_ARCH_RELEASE_DIR%\bullet\ /Y > NUL:
echo F | xcopy lib\Release\LinearMath.lib %LIB_ARCH_RELEASE_DIR%\bullet\ /Y > NUL:
touch %CHECKPOINT_DIR%\bullet_install
:bullet_install_done
...
cd %LLVM_NAME%
type %TOP%\Patches\Windows\%LLVM_NAME%.%ARCH%.*.patch | patch -p1 2> NUL:
cd ..
